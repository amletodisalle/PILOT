...
maldonado_output_file_binary = base_dir + 'word2vec/binary/MaldonadoDataset/df-maldonado-binary.csv'
maldonado_output_file_multiclass = base_dir + 'word2vec/multiclass/MaldonadoDataset/df-maldonado-multiclass.csv'
maldonado_output_folder_multiclass = base_dir + 'word2vec/multiclass/MaldonadoDataset/'
maldonado_output_folder_binary = base_dir + 'word2vec/binary/MaldonadoDataset/'

zhao_input_file = base_dir + 'datasets/zhao-dataset.csv'
zhao_output_file_binary = base_dir + 'word2vec/binary/ZhaoDataset/df-zhao-binary.csv'
zhao_output_folder_binary = base_dir + 'word2vec/binary/ZhaoDataset/'

w2v_path = base_dir + "word2vec/model/"
googleNews_vectors = w2v_path + 'GoogleNews-vectors-negative300.bin.gz'
...
def standardize(text):
    ...
    for w in nltk_tokens:
        if w not in stop_words:
            ...
            if not text.isdigit():
                # text = text.replace("'", "")
                l = wordnet_lemmatizer.lemmatize(text)
                # print("leem", text, l)
                result = result + ' ' + stemmer.stem(l)
                # print(result)
...
def word2vec(x_train, x_validation, x_test, output_folder, tokenizer, matrix):
    ...
    train_sequences = tf.keras.preprocessing.sequence.pad_sequences(tokenizer.texts_to_sequences(x_train), maxlen=MAX_LEN)
    validation_sequences = tf.keras.preprocessing.sequence.pad_sequences(tokenizer.texts_to_sequences(x_validation),
                                                                       maxlen=MAX_LEN)
    test_sequences = tf.keras.preprocessing.sequence.pad_sequences(tokenizer.texts_to_sequences(x_test), maxlen=MAX_LEN)

    print(train_sequences.shape, validation_sequences.shape, test_sequences.shape)
    print(train_sequences[0].shape, validation_sequences[0].shape, test_sequences[0].shape)
    # print(x_train)
...
def word2vec_tokenizer_matrix(comments):
    ...
    tokenizer = tf.keras.preprocessing.text.Tokenizer(lower=True, split=' ', num_words=max_words, oov_token="<pad>",
                                           filters='!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\t\n')
    ...
    w2v = models.KeyedVectors.load_word2vec_format(googleNews_vectors, binary=True)

    # Build weights of the embbeddings matrix using w2v
    emb_matrix = np.zeros((max_words, 300))
...
# BINARY_CLASSIFICATION = True uses the whole dataset, i.e., comments labelled with WITHOUT_CLASSIFICATION (Non-SATD) and all the other SATD types.
# BINARY_CLASSIFICATION = False means multi-class classification, i.e., only SATD comments are considered
# DATASET = 1 (Maldonado), DATASET = 2 (Zhao)
main(True, 1)
